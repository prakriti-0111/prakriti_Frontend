...
import { productFetch } from "actions/Retailer/product.actions";
import { AddToCart, CartList } from "actions/Retailer/addcart.actions";
import { WishListAdd } from "actions/Retailer/wishlist.actions";
...
import {RESET_WISHLIST} from 'actionTypes/Retailer/wishlist.type';
...
class ProductDetails extends React.Component {
    ...
    loadProduct = async () => {
        ...
        if(response.data.success){
            let stock_id = 0, size_name = '', product = response.data.data;
    ...
    handleSizeChange = (id) => {
        let stockIndex = this.state.stockIndex, size_name = '';
    ...
    handlePurityChange = (material_id, purity_id) => {
        ...
        let materialIndex = _.findIndex(stock.materials, function(item) { return item.id == material_id });
        let purityIndex = _.findIndex(stock.materials[materialIndex].purities, function(item) { return item.id == purity_id });
        ...
        stock.materials[materialIndex].price = stock.materials[materialIndex].purities[purityIndex].price;
        stock.materials[materialIndex].price_display = displayAmount(stock.materials[materialIndex].purities[purityIndex].price);
        let total_price = 0;
        ...
        stock.total_price = priceFormat(parseFloat(stock.making_charge) + total_price);
        stock.total_price_display = displayAmount(priceFormat(parseFloat(stock.making_charge) + total_price));
                                    ...
                                    <Container>
                                        <div className='breadcrumb-wrapper'>
                                            <Breadcrumb>
                                                ...
                                                <Breadcrumb.Item href=''>{product.category_name}</Breadcrumb.Item>
                                            ...
                                            <Col xs={12} md={5}>
                                                <div className='product-slider'>
                                                    <Swiper
                                                        ...
                                                        className="mySwiper2"
                                                    >
                                                        <SwiperSlide className='p_details_slider_wrapper' style={{maxHeight: '100px'}}>
                                                            {
                                                                ...
                                                                :
                                                                <img className='p_details_slider' src={product.images[this.state.imageIndex].path} />
                                                            }
                                                        </SwiperSlide>
                                                    </Swiper>
                                                    <Swiper
                                                    ...
                                                    >
                                                        {
                                                            ...
                                                            <SwiperSlide className='slider-thumbnail'>
                                                                <img className='thumnail-slider' src={videoThumb} onClick={() => this.playVideo()} />
                                                            </SwiperSlide>
                                                            : null
                                                        }
                                                        {
                                                            product.images.map((item, key) => (
                                                                <SwiperSlide className='slider-thumbnail' key={key}>
                                                                    <img className='thumnail-slider' src={item.path} onClick={() => this.changeSliderImage(key)} />
                                                                </SwiperSlide>
                                                            ))
                                                        }
                                                ...
                                                <div className='price-breakup'>
                                                    ...
                                                    <div className='breakup-content'>
                                                        {
                                                            product.stocks[this.state.stockIndex].materials.map((item, key) => (
                                                                <div className='breakup-item' key={key}>
                                                                    <span>{item.name}</span>  <span>{item.price_display}</span>
                                                                </div>
                                                            ))
                                                        }
                                                        
                                                        <div className='breakup-item'>
                                                            <span>Making Charge</span>  <span>{product.stocks[this.state.stockIndex].making_charge_display}</span>
                                                        </div>
                                                        {/*<div className='breakup-item'>
                                                        ...
                                                        <div className='breakup-item'>
                                                            <span><strong>Total</strong> </span>  <span><strong>{product.stocks[this.state.stockIndex].total_price_display}</strong> </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                                ...
                                                                <Accordion.Body>
                                                                    <div className='product-details-items-content'>
                                                                        <div className='product-details-items-item'>
                                                                            <span>Product Code</span>  <span>{product.product_code}</span>
                                                                        </div>
                                                                        {/*<div className='product-details-items-item'>
                                                                        ...
                                                                        <div className='product-details-items-item'>
                                                                            <span>Product Weight</span>  <span>{product.stocks[this.state.stockIndex].total_weight} gram</span>
                                                                        </div>
                                                                        {
                                                                            product.stocks[this.state.stockIndex].materials.map((item, key) => (
                                                                            <React.Fragment key={key}>
                                                                                <div className='product-details-items-item details-header'>
                                                                                    <span>{item.name.toUpperCase()} DETAILS</span>
                                                                                </div>
                                                                                <div className='product-details-items-item'>
                                                                                    <span>Total Weight</span>  <span>{item.total_gram} gram</span>
                                                                                </div>
                                                                                {
                                                                                    !isEmpty(item.quantity) ?
                                                                                    <div className='product-details-items-item'>
                                                                                        <span>Total No of {item.name.toUpperCase()}</span>  <span>{item.quantity}</span>
                                                                                    </div>
                                                                                    : null
                                                                                }
                                            ...
                                            <Col xs={12} md={7}>
                                                <div className='product-details-container'>
                                                    <div className='righ-side-wrapper'>
                                                        <div className='p-container-header'>
                                                            ...
                                                            <span className='share-icons'>
                                                                <p><BiHeart onClick={this.handleWishlist} /> </p><p> <BiShareAlt /></p>
                                                            </span>
                                                        </div>
                                                                    ...
                                                                    <Accordion.Body>
                                                                        {
                                                                            ...
                                                                            <div className='p-size'>
                                                                                ...
                                                                                <DropdownButton title={this.state.size_name} name='size'
                                                                                    key={'Secondary'}
                                                                                    ...
                                                                                    variant={'Secondary'}>
                                                                                    {
                                                                                        ...
                                                                                        (<div key={index}>
                                                                                            <Dropdown.Item onClick={() => this.handleSizeChange(item.stock_id)}>{item.size_name}</Dropdown.Item>
                                                                                        </div>)
                                                                                    )}
                                                                                </DropdownButton>
                                                                            </div>
                                                                                            ...
                                                                                            <div key={`inline-radio`}>
                                                                                                {
                                                                                                    item.purities.map((val, i) => (
                                                                                                        <Form.Check
                                                                                                            ...
                                                                                                            type={'radio'}
                                                                                                            onChange={() => this.handlePurityChange(item.id, val.id)}
                                                                                                        />
                                                                                                    ))
                                                                                                }
                                                    ...
                                                    <div className='product-buttons'>
                                                        <Button variant="secondary" onClick={this.handleAddToCart}>ADD to cart</Button>
                                                    </div>
                                                        ...
                                                        >
                                                            <SwiperSlide className='similar-slider-inner'>
                                                                ...
                                                                <div className='s-slider-content'>
                                                                    ...
                                                                    <div className="p-headder-offer-price"><span> <h2>₹ 9,999</h2></span><span className="disc"><img src={discount} alt='discount' /> <strong>66</strong></span></div>
...
const mapStateToProps = (state) => ({
    //product: state.retailer.product.product,
    //processing: state.retailer.product.processing,
    auth: state.auth,
    wl_actionCalled: state.retailer.wishlist.actionCalled,
    wl_createSuccess: state.retailer.wishlist.createSuccess,
    wl_successMessage: state.retailer.wishlist.successMessage,

    c_actionCalled: state.retailer.cart.actionCalled,
    c_createSuccess: state.retailer.cart.createSuccess,
    c_successMessage: state.retailer.cart.successMessage,
});

const mapDispatchToProps = dispatch => ({
    dispatch,
    actions: bindActionCreators({ productFetch, WishListAdd, AddToCart, CartList }, dispatch)
});
