...
import {AddressFetch, AddressList, getCountries, getStates, getDistricts} from "actions/Retailer/address.actions";
...
import {OrderCreate} from "actions/Retailer/placeOrder.actions";
import {toast} from "react-toastify";
import { RETAILER_ADDRESS_RESET } from 'actionTypes/Retailer/address.types';
import { RESET_ORDER } from 'actionTypes/Retailer/order.types';
...
class CheckoutPage extends React.Component {
    ...
    componentDidUpdate(prevProps){
        ...
        if(this.state.order_actionCalled){
            if(this.state.order_createSuccess){
                ...
                this.props.navigate('/retailer/order-success');
            }else{
    ...
    render(){
        const {newAddress, addressForm, addressFormErrors, addressList, countryList, stateList, districtList} = this.state;
...
const mapStateToProps = (state) => ({
    ...
    item_total: state.retailer.cart.item_total,
    item_total_display: state.retailer.cart.item_total_display,
    total_payable: state.retailer.cart.total_payable,
    ...
    order_actionCalled: state.retailer.order.actionCalled,
    order_createSuccess: state.retailer.order.createSuccess,
    order_successMessage: state.retailer.order.successMessage,
    order_errorMessage: state.retailer.order.errorMessage,
    order_id: state.retailer.order.order_id,
});

const mapDispatchToProps = dispatch => ({
    dispatch,
    actions: bindActionCreators({ AddressList, checkoutList, CartList, OrderCreate, AddressCreate}, dispatch)
});
